...
- **首要策略：防 AI 内容降权**。每天自动更新不能以数量优先；宁可少发、慢发，也不要发布像批量 AI 生成的泛泛旅游文章。任何 Codex/Claude/自动化任务都必须先满足这一条。
- **内容必须有编辑判断**：每篇文章要有真实旅行约束、失败场景、备用方案、具体时间/费用/限制口径、适用人群和“不适合什么情况”。不要只写通用概念和漂亮段落。
- **禁止明显 AI 套话**：避免 “let's dive in”、泛泛的 “in conclusion”、过度 em dash、rule-of-three 堆砌、宣传册语气、vibrant/rich/stunning 这类空泛形容、以及 “experts say” 这类无来源归因。
- **事实安全优先**：支付、签证、交通、通信、健康/安全相关内容不能虚构价格、政策、限额或 App 行为。规则会变化时，要写清楚变化点并提醒读者核对官方或服务商信息。
...
- **质量抽检**：每周人工抽读 2–3 篇，检查事实准确性、是否有 AI 套话、内链/联盟占位是否自然。若 GSC 收录停滞，先暂停自动发布并重写已发布低质页。
- **监控指标**：GSC 的收录数、展示/点击、平均排名；Clarity 的滚动深度、停留、点击和录屏。收录停滞或跳出飙高时先降频、提质。
